...
void KeyAuth::api::init()
{
    ...
    //std::cout << "SentKey       = " << sentKey << "\n"; // used for debugging - confirm everything matches
    //std::cout << "Client enckey = " << enckey << "\n";
...
std::string KeyAuth::api::req(std::string data, std::string url) {
    ...
    //curl_easy_setopt(curl, CURLOPT_VERBOSE, 1L); used for debugging purposes
...
void error(std::string message) {
    MessageBoxA(nullptr, message.c_str(), "Erro", MB_OK | MB_ICONERROR | MB_TOPMOST);
    ExitProcess(0);
}
