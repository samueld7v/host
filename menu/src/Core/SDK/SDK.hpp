...
namespace Core {
	...
	namespace SDK {
		...
		namespace Game
		{
			...
			inline D3DXVECTOR2 WorldToScreen( D3DXVECTOR3 World, const D3DXMATRIX& ViewMatrix )
			{
			...
			{
				static D3DXMATRIX cachedMatrix;
				static int lastFrame = -1;
				int currentFrame = ImGui::GetFrameCount();
				if ( currentFrame != lastFrame ) {
					D3DXMATRIX raw = Mem.Read<D3DXMATRIX>( Core::SDK::Pointers::pViewPort + 0x24C );
					D3DXMatrixTranspose( &cachedMatrix, &raw );
					lastFrame = currentFrame;
				}
				return WorldToScreen( World, cachedMatrix );
			}
			...
			inline bool GetBoneIndex( cSkeleton_t cSkeleton, unsigned int boneId, int & outBoneIdx )
			{
				...
				auto it = StoredBonesIdxCache.find( boneId );
				if ( it != StoredBonesIdxCache.end( ) ) {
					outBoneIdx = it->second;
					return true;
				}
