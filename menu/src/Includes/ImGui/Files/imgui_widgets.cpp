...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
	...
	window->DrawList->AddRectFilledMultiColor(grab_rect.Min, grab_rect.Max, GetColorU32(g_Col.Base), GetColorU32(g_Col.BaseEnd), GetColorU32(g_Col.BaseEnd), GetColorU32(g_Col.Base), 8.f);
	return held;
}
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
	...
	// Verifica se est� em estado de hover
	...
	if (hovered)
	{
		...
		// Aqui voc� pode aplicar um efeito de blur na imagem usando t�cnicas espec�ficas do ImGui, como shaders, se estiver implementando shaders personalizados
	}
	else
	{
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	Window->DrawList->AddRectFilledMultiColor(
		...
		ImGui::GetColorU32(g_Col.BaseEnd),
		ImGui::GetColorU32(g_Col.BaseEnd),
		ImGui::GetColorU32(g_Col.Base),
		5
	);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
	...
	const float width = size_arg.x > 0.0f ? size_arg.x : GetWindowContentRegionMax().x - GetWindowContentRegionMin().x - (window->ScrollbarY ? 5.f : 0.f);
	...
	const char* text_display_end = FindRenderedTextEnd(desc_id);
	if (desc_id < text_display_end)
		window->DrawList->AddText(g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, rect.Min - ImVec2(1, 4), GetColorU32(g_Col.SecundaryFeaturesText), desc_id, text_display_end);
...
bool ImGui::ListSelectableEx(const char* Label, bool Selected)
{
	...
	struct Selectable_t {
		...
		ImVec4 BackGroundCol = (ImVec4)ImColor(25, 25, 25);
	...
	SelectableWidget->second.TextCol = ImLerp(SelectableWidget->second.TextCol, Selected ? ImVec4(g_Col.Base.x, g_Col.Base.y, g_Col.Base.z, 1.f) : (ImVec4)ImColor(g_Col.SecundaryFeaturesText), g.IO.DeltaTime * 8.f);
	SelectableWidget->second.BackGroundCol = ImLerp(SelectableWidget->second.BackGroundCol, Selected || Hovered ? (ImVec4)ImColor(35, 35, 35) : (ImVec4)ImColor(25, 25, 25), g.IO.DeltaTime * 8.f);
	...
	Window->DrawList->AddRect(Rect.Min, Rect.Max, GetColorU32(ImVec4(ImColor(35, 35, 35))), 6);
