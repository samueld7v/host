...
// skCrypt compatibility macro for user-provided credentials
template <typename T>
struct skCryptWrapper {
    T obj;
    skCryptWrapper(T&& o) : obj(std::move(o)) {}
    auto decrypt() { return obj.crypt_get(); }
};

#define skCrypt(str) skCryptWrapper(xorstr_(str))
