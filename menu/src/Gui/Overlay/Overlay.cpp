...
#include <Includes/ImGui/MinecraftFont.hpp>
#include <Core/Core.hpp>
#include <Core/Features/Player.hpp>
#include <Gui/Gui.hpp>
#include <dwmapi.h>
#include <tchar.h>
#include <thread>
...
#include <Includes/kid_image.hpp>
...
namespace Gui
{
	...
	static void UpdateMenuSizeForResolution(float gameW, float gameH)
	{
		float baseW = 1920.f, baseH = 1080.f;
		float scaleX = gameW / baseW;
		float scaleY = gameH / baseH;
		float scale = min(scaleX, scaleY);
		scale = max(0.5f, min(scale, 2.5f));
		g_MenuInfo.MenuSize = ImVec2(980.f, 430.f);
		g_MenuInfo.LoginSize = ImVec2(800.f, 380.f);
		g_MenuInfo.ResolutionScale = scale;
	}

	void Overlay::Render()
	{
		UpdateMenuSizeForResolution(g_Variables.g_vGameWindowSize.x, g_Variables.g_vGameWindowSize.y);
		...
		if (!g_MenuInfo.IsOpen) dwExStyle |= WS_EX_TRANSPARENT;
		g_Variables.g_hCheatWindow = CreateWindowExW(dwExStyle, wc.lpszClassName, L" ", WS_POPUP, g_Variables.g_vGameWindowPos.x, g_Variables.g_vGameWindowPos.y, g_Variables.g_vGameWindowSize.x, g_Variables.g_vGameWindowSize.y, NULL, NULL, wc.hInstance, NULL);
#endif // FULLSCREEN_EX		

		SetLayeredWindowAttributes(g_Variables.g_hCheatWindow, RGB(0, 0, 0), 255, LWA_ALPHA);
		MARGINS Margin = { 0, 0, static_cast<int>(g_Variables.g_vGameWindowSize.x), static_cast<int>(g_Variables.g_vGameWindowSize.y) };
		...
		std::thread([&]() { NotifyManager::Send(xorstr("Injetado!"), 4000); }).detach();
		...
		while (Message.message != WM_QUIT)
		{
...
#ifdef FULLSCREEN_EX
						SetWindowLong(g_Variables.g_hCheatWindow, GWL_EXSTYLE, WS_EX_TOPMOST | WS_EX_NOACTIVATE);
#else
			...
			g_MenuInfo.MenuAlpha = ImLerp(g_MenuInfo.MenuAlpha, g_MenuInfo.IsOpen ? 1.f : 0.f, ImGui::GetIO().DeltaTime * 8);

			bool NeedsRendering = g_MenuInfo.IsOpen || g_MenuInfo.MenuAlpha > 0.01f ||
				g_Config.ESP.Enabled ||
				g_Config.Aimbot.ShowFov ||
				g_Config.SilentAim.ShowFov ||
				g_Config.TriggerBot.ShowFov ||
				g_Config.General.WaterMark ||
				!NotifyManager::NotifyList.empty() ||
				g_Config.Player.FreecamEnabled ||
				g_Config.ESP.Crosshair ||
				g_Config.MiscESP.FeatureList ||
				g_Config.ESP.AdminList ||
				g_Config.ESP.InvisiblePlayerWarning ||
				g_Config.VehicleESP.Enabled ||
				g_Config.Player.WeatherControlEnabled;
			...
			if (NeedsRendering) {
				...
				ImGui::NewFrame();
				{
					if (g_MenuInfo.MenuAlpha > 0.01f) {
						float dimAlpha = g_MenuInfo.Glassmorphism ? 0.18f : 0.4f;
						float currentDim = g_MenuInfo.MenuAlpha * dimAlpha;
						ImGui::GetBackgroundDrawList()->AddRectFilled(ImVec2(0, 0), ImVec2(g_Variables.g_vGameWindowSize.x, g_Variables.g_vGameWindowSize.y), ImColor(0.f, 0.f, 0.f, currentDim));
					}
					Gui::Rendering();
				...
				g_pSwapChain->Present(g_Config.General.VSync ? 1 : 0, 0U);
			}
			else {
				WaitMessage();
			}
	...
	void Overlay::Fonts()
	{
		...
		g_Variables.m_FontGiant = io.Fonts->AddFontFromMemoryTTF(InterSemiBold, sizeof(InterSemiBold), 28.f, &Cfg, io.Fonts->GetGlyphRangesCyrillic());
		...
		g_Variables.m_MinecraftFont = io.Fonts->AddFontFromMemoryTTF((void*)MinecraftFont_data, sizeof(MinecraftFont_data), 16.f, &Cfg, io.Fonts->GetGlyphRangesCyrillic());
	...
	void Overlay::Style()
	{
		...
		if (g_Variables.Logo == nullptr)
		{
			...
			D3DX11CreateShaderResourceViewFromMemory(g_pd3dDevice, kid_png, sizeof(kid_png), &Info, Pump, &g_Variables.kid_image, 0);
			//WeaponsIcons
