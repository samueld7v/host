...
#include "Features/VehicleColor.hpp"
#include "Features/Player.hpp"
#include "Features/Exploits/PsychicGun.hpp"
...
namespace Core {
	...
	inline bool IsPedReady(CPed* ped)
	{
		if (!ped) return false;
		if (!ped->GetNavigation()) return false;
		if (!ped->GetWeaponManager()) return false;
		return true;
	}

	enum class CVehicleModelInfoFlags : uint16_t {
		JUMPING_CAR = 43
	};

	inline void SetVehicleModelFlag(CVehicleModelInfoFlags Flag, bool Toggle)
	{
		using namespace Core::SDK::Pointers;
		if (!pLocalPlayer) return;
		CVehicle* lastVeh = pLocalPlayer->GetLastVehicle();
		if (!lastVeh) return;

		uint64_t VehicleModelInfo = lastVeh->GetModelInfo();
		if (!VehicleModelInfo)
			return;

		uint64_t CVehicleModelInfo = VehicleModelInfo + 0x057C;
		uint16_t Index = static_cast<uint16_t>(Flag);
		uint32_t CurrentFlags = Mem.Read<uint32_t>(CVehicleModelInfo + (Index / 32) * 4);

		if (Toggle)
			CurrentFlags |= (1 << (Index % 32)); // Ativa a flag (JUMPING_CAR)
		else
			CurrentFlags &= ~(1 << (Index % 32)); // Desativa a flag (JUMPING_CAR)

		Mem.Write<uint32_t>(CVehicleModelInfo + (Index / 32) * 4, CurrentFlags);
	}

	inline void StartSelfThreads()
	{
		using namespace Core::SDK::Pointers;
		...
		{
			int loopDelayMs = 30;
			CPed* localPlayer = pLocalPlayer;

			if (!IsPedReady(localPlayer)) {
			...
			{
				CVehicle* vehicle = localPlayer->GetLastVehicle();
				if (vehicle && g_Config.Player.BuzinaBoost)
				{
					loopDelayMs = 5;
					if (g_Config.Player.BuzinaBoostKey != 0 && (GetAsyncKeyState(g_Config.Player.BuzinaBoostKey) & 0x8000))
					{
						uint8_t flags29 = Mem.Read<uint8_t>(reinterpret_cast<uintptr_t>(vehicle) + 0x29);
						if (!(flags29 & 1))
							Mem.Write<uint8_t>(reinterpret_cast<uintptr_t>(vehicle) + 0x29, flags29 | 1);

						uint8_t flags97F = Mem.Read<uint8_t>(reinterpret_cast<uintptr_t>(vehicle) + 0x97F);
						if (!(flags97F & 0x80))
							Mem.Write<uint8_t>(reinterpret_cast<uintptr_t>(vehicle) + 0x97F, flags97F | 0x80);

						Mem.Write<float>(reinterpret_cast<uintptr_t>(vehicle) + 0xAD4, g_Config.Player.BuzinaBoostSpeed);
					}
				}

				if (g_Config.Player.SeatBelt)
				{
					localPlayer->SeatBealt(true);
				}
				else
				{
					localPlayer->SeatBealt(false);
				}

				if (vehicle)
				{
					uint64_t modelInfo = vehicle->GetModelInfo();
					if (modelInfo)
					{
						// Jumping Car
						static bool LastJumpingCarState = false;
						if (LastJumpingCarState != g_Config.Player.JumpingCar)
						{
							SetVehicleModelFlag(CVehicleModelInfoFlags::JUMPING_CAR, g_Config.Player.JumpingCar);
							LastJumpingCarState = g_Config.Player.JumpingCar;
						}

						// Car Planing (Parachute)
						uint16_t currentFlags = Mem.Read<uint16_t>(modelInfo + 0x58B);
						if (g_Config.Player.CarPlaning)
						{
							if (!(currentFlags & (1 << 8)))
								Mem.Write<uint16_t>(modelInfo + 0x58B, currentFlags | (1 << 8));
						}
						else
						{
							if (currentFlags & (1 << 8))
	...
	inline void StartThreads() {
		...
		std::thread(&StartSelfThreads).detach();
		...
		std::thread(&Features::cVehicleColor::Start, &Features::g_VehicleColor).detach();
		std::thread(&Features::cPsychicGun::Start, &Features::g_PsychicGun).detach();
	...
	inline int GetBuild() {
		std::wstring iniFilePath = _wgetenv(L"LOCALAPPDATA");
		iniFilePath += L"\\FiveM\\FiveM.app\\CitizenFX.ini";
	...
	inline bool SetupOffsets() {
		...
		// Extrai a build diretamente do nome do executável
		std::wsmatch match;
		std::wregex pattern(L"FiveM_b(\\d+)_(GTA|Game)Process\\.exe");

		if (std::regex_match(Mem.ProcName, match, pattern) && match.size() > 1) {
			g_Offsets.CurrentBuild = std::stoi(match[1].str());
		}
		else if (Mem.ProcName == L"FiveM_GTAProcess.exe" || Mem.ProcName == L"FiveM_GameProcess.exe") {
		...
		case 3258:
			...
			g_Offsets.m_DamageStruct = 0x0;
			g_Offsets.m_NumberOfWheels = 0xC28;
		...
		case 3095:
			...
			g_Offsets.m_DamageStruct = 0x0;
			g_Offsets.m_NumberOfWheels = 0xBC0;
			g_Offsets.m_VisibleFlag = 0x145C;
		...
		case 2802:
			...
			g_Offsets.m_DamageStruct = 0x0;
			g_Offsets.m_NumberOfWheels = 0xBBC;
			g_Offsets.m_VisibleFlag = 0x145C;
		...
		case 2699:
			...
			g_Offsets.m_DamageStruct = 0x0;
			g_Offsets.m_NumberOfWheels = 0xBA4;
			g_Offsets.m_VisibleFlag = 0x147C;
		...
		if (g_Offsets.CurrentBuild != 0)
		{
			//Functions Signatures
			{
				...
				g_Offsets.m_CopyVariation = Mem.FindSignature(
					{ 0x48, 0x89, 0x5C, 0x24, 0x00, 0x48, 0x89, 0x6C, 0x24, 0x00, 0x48, 0x89, 0x74, 0x24, 0x00, 0x57, 0x41, 0x56, 0x41, 0x57, 0x48, 0x81, 0xEC, 0x00, 0x00, 0x00, 0x00, 0x49, 0x8B, 0xE8 }
				);

				g_Offsets.m_SetInSpectatorMode = Mem.FindSignature(
					{ 0x40, 0x53, 0x48, 0x83, 0xEC, 0x00, 0x84, 0xC9, 0x75, 0x00, 0x33, 0xC9 }
				);

				g_Offsets.m_IsInSpectatorMode = Mem.FindSignature(
					{ 0x40, 0x53, 0x48, 0x83, 0xEC, 0x00, 0x33, 0xDB, 0x38, 0x1D, 0x00, 0x00, 0x00, 0x00, 0xE9 }
				);

				g_Offsets.m_SetVehicleForwardSpeed = Mem.FindSignature(
					{ 0x48, 0x83, 0xEC, 0x38, 0x0F, 0x29, 0x74, 0x24, 0x00, 0x0F, 0x28, 0xF1, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x48, 0x85, 0xC0, 0x74, 0x00, 0xF6, 0x40 }
				);

				g_Offsets.m_PointerToHandle = Mem.FindSignature(
					{ 0x48, 0x89, 0x5C, 0x24, 0x00, 0x48, 0x89, 0x74, 0x24, 0x00, 0x57, 0x48, 0x83, 0xEC, 0x20, 0x8B, 0x15, 0x00, 0x00, 0x00, 0x00, 0x48, 0x8B, 0xF9, 0x48, 0x83, 0xC1, 0x10, 0x33, 0xDB }
				);
				...
				//f3 0f 11 47 ? f3 0f 10 44 24 ? f3 0f 11 4f ? f3 0f 10 4c 24 ? f3 0f 11 47 ? f3 0f 11 4f ? e8
				g_Offsets.m_FreecanPatch = Mem.FindSignature(
					{ 0xf3, 0x0f, 0x11, 0x47, 0x00, 0xf3, 0x0f, 0x10, 0x44, 0x24, 0x00, 0xf3, 0x0f, 0x11, 0x4f, 0x00, 0xf3, 0x0f, 0x10, 0x4c, 0x24, 0x00, 0xf3, 0x0f, 0x11, 0x47, 0x00, 0xf3, 0x0f, 0x11, 0x4f, 0x00, 0xe8 }
				);

				//48 89 5C 24 08 48 89 6C 24 10 48 89 74 24 18 57 48 83 EC 20 41 8A F0 8B DA 8B F9
				g_Offsets.m_PatchButt = Mem.FindSignature(
					{ 0x48, 0x89, 0x5C, 0x24, 0x00, 0x48, 0x89, 0x6C, 0x24, 0x00, 0x48, 0x89, 0x74, 0x24, 0x00, 0x57, 0x48, 0x83, 0xEC, 0x00, 0x41, 0x8A, 0xF0, 0x8B, 0xDA, 0x8B, 0xF9 }
				);

				// E8 ?? ?? ?? ?? 44 0F 11 40 ?? 48 8B 43
				g_Offsets.m_InvisibleAddress = Mem.FindSignature(
					{ 0xE8, 0x00, 0x00, 0x00, 0x00, 0x44, 0x0F, 0x11, 0x40, 0x00, 0x48, 0x8B, 0x43 }
				);

				// 8B 05 ?? ?? ?? ?? 41 89 42 ?? F3 0F 10 05 (WeatherTimePointer)
				uintptr_t WeatherTimeSig = Mem.FindSignature(
					{ 0x8B, 0x05, 0x00, 0x00, 0x00, 0x00, 0x41, 0x89, 0x42, 0x00, 0xF3, 0x0F, 0x10, 0x05 }
				);
				if (WeatherTimeSig) {
					int offset = Mem.Read<int>(WeatherTimeSig + 2);
					g_Offsets.m_WeatherTimePointer = WeatherTimeSig + 6 + offset;
				}

				g_Offsets.m_SoloSessionStart = Mem.FindSignature(
					{ 0x48, 0x83, 0xEC, 0x00, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x48, 0x85, 0xC0, 0x74, 0x00, 0x48, 0x8B, 0x88, 0x00, 0x00, 0x00, 0x00, 0x48, 0x85, 0xC9, 0x74, 0x00, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x48, 0x83, 0xC4 }
				);

				g_Offsets.m_SoloSessionEnd = Mem.FindSignature(
					{ 0x48, 0x83, 0xEC, 0x00, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x48, 0x85, 0xC0, 0x74, 0x00, 0x48, 0x8B, 0x88, 0x00, 0x00, 0x00, 0x00, 0x48, 0x85, 0xC9, 0x74, 0x00, 0x80, 0xB9 }
				);

				//48 8b c4 48 89 58 ? 48 89 68 ? 48 89 70 ? 57 41 56 41 57 48 83 ec ? 80 3d ? ? ? ? ? 0f 29 78
				g_Offsets.m_BreakOffWheel = Mem.FindSignature({ 0x48, 0x8b, 0xc4, 0x48, 0x89, 0x58, 0x00, 0x48, 0x89, 0x68, 0x00, 0x48, 0x89, 0x70, 0x00, 0x57, 0x41, 0x56, 0x41, 0x57, 0x48, 0x83, 0xec, 0x00, 0x80, 0x3d, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x29, 0x78 });

				//48 8B 15 ? ? ? ? 48 C1 E1 - player_name pattern
				{
					std::vector<std::wstring> modsToSearch = {
						L"citizen-playernames-five.dll",
						L"FiveM_b3258_GTAProcess.exe",
						L"FiveM_GTAProcess.exe",
						L"FiveM_GameProcess.exe",
						L"GTA5.exe",
					};
					for (auto& modName : modsToSearch) {
						uint64_t modBase = Mem.GetModuleBaseByName(g_Variables.ProcIdFiveM, modName);
						uint64_t modSize = Mem.GetModuleBaseSizeByName(g_Variables.ProcIdFiveM, modName);
						if (modBase && modSize) {
							uintptr_t sig = Mem.FindSignature(
								{ 0x48, 0x8B, 0x15, 0x00, 0x00, 0x00, 0x00, 0x48, 0xC1, 0xE1 },
								modBase, modSize
							);
							if (sig) {
								uintptr_t globalAddr = Mem.ResolveRelativeAddress(sig, 7);
								uintptr_t namesTable = Mem.Read<uintptr_t>(globalAddr);
								if (namesTable) {
									g_Offsets.m_PlayerNamePtr = namesTable;
									{
										int wlen = (int)modName.size();
										int alen = WideCharToMultiByte(CP_UTF8, 0, modName.c_str(), wlen, NULL, 0, NULL, NULL);
										std::string modNameA(alen, 0);
										WideCharToMultiByte(CP_UTF8, 0, modName.c_str(), wlen, &modNameA[0], alen, NULL, NULL);
										std::cout << "[NameScan] Found pattern in " << modNameA << " -> m_PlayerNamePtr = 0x" << std::hex << namesTable << std::dec << std::endl;
									}
									break;
								}
							}
